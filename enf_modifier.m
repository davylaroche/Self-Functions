function enf_modifier( varargin )
% this function read and write enf eclispe vicon file
% varargin{1} must be the file name
% var need to be varargin{2} = key type ('node information' or 'trial_info' or 'lock_info')
% var need to be varargin{3} = key name ('description', 'stages', 'notes')
% var need to be varargin{4} = key infos : 
% - for stages :  'Unclassified', 'Static', 'Good Left', 'Good Right', 'Good Both', 'No Kinetics', 'Not for use'
% - for description preferably : 'PiG Processed' or other KIN model & 'cycles selected'
% - for notes what you want in string format

%% Verification of input
if rem((length(varargin)-1), 3)~=0;
    error 'not all fields have been provided'
else
    infos = varargin(2:end);
    fields = length(infos)/3;
end
...
for c1 = 1:fields
    ...
    key1 = cellfindi(keys(:, 1), infos{3*c1-2});
    key2 = cellfindi(keys(key1, 3), infos{3*c1-1});
    if ~isempty(key2)
        keys(key1(key2), 4) = {char(keys(key1(key2), 4)), ' ; ', char(infos(3*c1))};
    else
        keys(end+1, 1) = infos(3*c1-2);
        keys(end, 3) = infos(3*c1-1);
        keys(end, 4) = infos(3*c1);
    end
end

%% Parcourir le fichier pour combler les cellules vides
for c2 = 1:size(keys, 1)
    for c3 = 1:size(keys, 2)
        try
            if isempty(char(keys(c2, c3))), keys(c2, c3) = {''};end
        end
        try
            if isempty(cell2mat(keys(c2, c3))), keys(c2, c3) = {''};end
        end
    end
end

%% Writing enf file
inifile(fName,'write', keys, 'plain');
